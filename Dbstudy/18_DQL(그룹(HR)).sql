-- 그룹(GROUP BY)
-- 1. GROUP BY절에서 지정한 칼럼의 데이터는 하나로 모아서 한 번만 조회가 가능
...
SELECT EMPLOYEE_ID  --> 이걸 조회하고 싶으면
  FROM EMPLOYEES
 GROUP BY DEPARTMENT_ID; --> 여기에도 EMPLOYEE_ID가 있어야함


-- 3. 그룹화 함수 사용
-- 집계함수(그룹함수)는 GROUP BY절에 해당 칼럼이 없어도 SELECT절에서 조회 가능
SELECT 
       DEPARTMENT_ID
...
--    3) HAVING절 : WHERE절에서 처리할 수 없는 조건만 HAVING절로 처리함

-- 1) 부서번호가 100 미만인 부서들의 연봉평균을 조회하기(WHERE절, HAVING절 모두 가능한 조건)
-- WHERE절(권장) : 성능이 더 우수한 쿼리 
SELECT
       DEPARTMENT_ID
     , FLOOR(AVG(SALARY)) AS 부서별연봉평균
  FROM 
...
-- 2) 소속된 사원수가 10명 이상인 부서의 연봉평균 조회하기(HAVING절에서만 가능한 조건)
...
--    부서번호가 없는 사원은 제외하기
--> GROUP BY에 있는 둘중 하나만 SELECT에 넣어도 조회가능
...
-- ※DEPARTMENTS 테이블 연습※
-- 1. 동일한 지역(LOCATION_ID)로 그룹화하여 조회하기
SELECT LOCATION_ID 
  FROM DEPARTMENTS
 GROUP BY LOCATION_ID;

-- 2. 동일한 지역(LOCATION_ID)로 그룹화하여 각 지역별 존재하는 부서수 조회하기
--    부서수가 2 이상인 지역만 조회하기
...
-- 3. 부서명(DEPARTMENT_NAME)의 첫 2글자로 그룹화하여 해당 그룹의 개수 조회하기
--> GROUP BY에 함수들어가있으면 SELECT에 그대로 함수 쓰면 됨
