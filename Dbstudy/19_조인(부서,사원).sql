...
-- 참조무결성(을) 위배 데이터 삽입을 위해서 외래키 일시중지
ALTER TABLE EMPLOYEE
...
/* 
    ...
    4. 종류
       1) 크로스 조인 : 카테젼 곱, 각 테이블의 모든 행을 조인
          ...
          - 조인 조건을 잘못 지정한 경우 (조인 조건이 안돼서 크로스 조인이 되어버릴 수 있음)
       2) 내부 조인 
          - INNER JOIN
          - 각 테이블에서 일치하는 모든 행을 조인
       3) 외부 조인
          - OUTER JOIN
          - 한 테이블은 일치하는 행을 조인, 한 테이블은 일치여부와 상관없이 조인
          - 왼쪽 외부조인(LEFT OUTER JOIN), 오른쪽 외부 조인(RIGHT OUTER JOIN)
       4) 셀프 조인
          - SELF JOIN
          - 한 테이블에 참조 관계가 있는 경우
...
SELECT E.EMP_NO, E.NAME, E.SALARY, D.DEPT_NO, D.DEPT_NAME
  FROM DEPARTMENT D CROSS JOIN EMPLOYEE E;  --> 결과가 쓰레기임 (4개부서 X 5명직원 = 20개의 데이터)
  
-- 2. 내부 조인 확인
--    사원번호, 사원명, 부서명을 조회하기
SELECT E.EMP_NO, E.NAME, D.DEPT_NO
...
--   모두포함시킬 사원테이블을 OUTER의 왼쪽/오른쪽에 두느냐에 따라 
