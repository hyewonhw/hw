...
--     KOR + ENG + MATH 안되는 이유 -> NULL값이 포함되어 있으면 결과가 NULL값
  FROM 
       SAMPLE;

-- 2. NVL2 함수 (IF ELSE 재질)
--    NVL2(칼럼, NULL이 아닐 때 사용할 값, NULL일 때 사용할 값)
SELECT
       NVL2(NAME, NAME || '님', '아무개')    --> 문자열 연결 연산자 (JAVA에서의 +)
     , NVL2(KOR, '응시', '결시')
     , NVL2(ENG, '응시', '결시')
     , NVL2(MATH, '응시', '결시')
  FROM 
...
-- 각 칼럼(KOR,ENG,MATH)의 합계 (=세로 // 로우(=가로)단위연산X)
SELECT 
       SUM(KOR)  -- 국어합
     , SUM(ENG)  -- 영어합
     , SUM(MATH) -- 수학합
