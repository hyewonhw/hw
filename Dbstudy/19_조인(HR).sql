...
SELECT E.EMPLOYEE_ID, E.LAST_NAME, D.DEPARTMENT_NAME
  FROM DEPARTMENTS D INNER JOIN EMPLOYEES E  -- 부모먼저, 자식나중
    ON D.DEPARTMENT_ID = E.DEPARTMENT_ID  -- 조인(을 위한) 조건 / 앞이 PK(DRIVE) 뒤가 FK(DRIVEN)
 WHERE D.LOCATION_ID = 1700;              -- 일반 조건(조인과 상관없음)

-- COMMA
SELECT E.EMPLOYEE_ID, E.LAST_NAME, D.DEPARTMENT_NAME
  FROM DEPARTMENTS D, EMPLOYEES E           -- INNER JOIN없으면 ON 사용불가
 WHERE D.DEPARTMENT_ID = E.DEPARTMENT_ID    -- 일반조건으로 변경
   AND D.LOCATION_ID = 1700;                -- AND 연산자로 연결
...
-- JOB_HISTORY 테이블의 JOB_ID(PK) = E.JOB_ID(FK)
...
--    EMPLOYEE_ID의 오름차순 정렬로 조회하시오. --> ASC
-- DEPARTMENTS 테이블 : DEPARTMENT_NAME
-- EMPLOYEES   테이블 : EMPLOYEE_ID, LAST_NAME
-- LOCATIONS   테이블 : CITY
...
SELECT E.EMPLOYEE_ID, E.LAST_NAME, D.DEPARTMENT_NAME, L.CITY, C.COUNTRY_NAME
  FROM COUNTRIES C INNER JOIN LOCATIONS L
    ...
    ON D.DEPARTMENT_ID = E.DEPARTMENT_ID;  -- 조인조건(NULL 제외된 상태로 처리됨)
...
SELECT E.EMPLOYEE_ID, E.LAST_NAME, NVL(D.DEPARTMENT_NAME, 'None')   --> NULLVALUE함수사용
  FROM DEPARTMENTS D RIGHT OUTER JOIN EMPLOYEES E
    ON D.DEPARTMENT_ID = E.DEPARTMENT_ID
 ORDER BY E.EMPLOYEE_ID;  --> 오름차순을 위해 추가

-- COMMA
SELECT E.EMPLOYEE_ID, E.LAST_NAME, NVL(D.DEPARTMENT_NAME, 'None')
  FROM DEPARTMENTS D, EMPLOYEES E
 WHERE D.DEPARTMENT_ID(+) = E.DEPARTMENT_ID  --> 오른쪽조인이여서 왼쪽에 (+) / 왼쪽조인이면 오른쪽에(+)
 ORDER BY E.EMPLOYEE_ID; 
