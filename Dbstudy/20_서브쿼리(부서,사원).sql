...
/*
    ...
    1. SUB QUERY
    2. 메인 쿼리(MAIN QUERY)에 포함하는 하위쿼리(SUB QUERY)
...
SELECT EMP_NO, NAME, DEPART, GENDER, HIRE_DATE, SALARY
  FROM EMPLOYEE
 WHERE POSITION = (SELECT POSITION         -- 단일 행 서브쿼리이므로 연산자 =를 사용, 하위쿼리는 메인쿼리와 동등비교(=)되므로 반드시 POSITION을 반환시켜줘야함
                     FROM EMPLOYEE
                    WHERE EMP_NO = 1001);  -- EMP_NO는 PK이므로 단일 행 서브쿼리
--> 메인에서 POSITION이면 하위에서 POSITION반환
...
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE
 WHERE DEPART IN(SELECT POSITION
                   FROM DEPARTMENT
                  WHERE DEPT_NAME = '영업부');  -- DEPT_NAME가 PK/UNIQUE가 아니기 때문에 다중 행 서브쿼리

-- INNER JOIN
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE D INNER JOIN EMPLOYEE E
...
SELECT DEPT_NO, DEPT_NAME, LOCATION
 ...
 WHERE DEPT_NO IN(SELECT DEPART
                    FROM EMPLOYEE
                   WHERE POSITION = '과장');  -- DEPT_NAME가 PK/UNIQUE가 아니기 때문에 다중 행 서브쿼리
...
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
 ...
 WHERE SALARY > ANY(SELECT SALARY
                      FROM EMPLOYEE
                     WHERE DEPART = 1);   -- DEPTART가 PK/UNIQUE가 아니기 때문에 다중 행 서브쿼리
...
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
 ...
 WHERE SALARY > (SELECT MIN(SALARY)
                   FROM EMPLOYEE
                  WHERE DEPART = 1);    -- 서브쿼리가 (집계)함수(MIN)이므로 단일 행 서브쿼리
...
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
 ...
 WHERE SALARY > (SELECT MAX(SALARY)
                   FROM EMPLOYEE
                  WHERE DEPART = 1);    -- 서브쿼리가 (집계)함수(MAX)이므로 단일 행 서브쿼리
...
/*
  ...
  4. 종류 
     ...
     2) ROWNUM : 행(ROW)의 NUMBER, 어떤 행의 번호 (가상칼럼)
*/
...
SELECT EMP_NO, NAME
  FROM EMPLOYEE
 WHERE ROWID = 'AAAFCeAABAAADURAAD'; 
...
SELECT EMP_NO, NAME
  FROM EMPLOYEE
 WHERE ROWNUM BETWEEN 1 AND 2;   --> 조회가능

SELECT EMP_NO, NAME
  FROM EMPLOYEE
 WHERE ROWNUM = 2;  --> 1번을 포함하지 않는 조건이라 데이터 못가져옴 / 조회불가능

-- ROWNUM을 1 이외의 범위를 조건으로 사용하는 방법
-- ROWNUM에 별명을 지정하고 해당 별명을 사용하면 됨
SELECT ROWNUM AS ROW_NUM, EMP_NO, NAME  --> 별명을 ROW_NUM으로 지정
  FROM EMPLOYEE 
 WHERE ROW_NUM = 2;   --> 실행순서상 별명지정이 마지막이기 때문에 실행불가 / 인라인뷰 사용

--> 인라인뷰 사용
SELECT A.ROW_NUM, A.EMP_NO, A.NAME  --> A.ROW_NUM생략
  FROM (SELECT ROWNUM AS ROW_NUM, EMP_NO, NAME
          FROM EMPLOYEE) A   --> 인라인뷰 별명 A / A테이블의 세가지 정보를 메인 쿼리에서 사용가능
 WHERE A.ROW_NUM = 2;
...
-- 2) 정렬과 ROWNUM 이용
--    (1) 연봉의 내림차순 정렬을 수행(가장 높은 연봉이 1번째 행)
--    (2) 정렬 결과에서 ROWNUM = 1인 행을 조회
SELECT ROWNUM, A.EMP_NO, A.NAME, A.SALARY  --> (2) 정렬 후 번호 부여
  FROM (SELECT EMP_NO, NAME, SALARY
          FROM EMPLOYEE
         ORDER BY SALARY DESC) A  --> (1) 내림차순 완료
 WHERE ROWNUM = 1;
...
-- 2) ★ROW_NUMBER() 함수 
--    정렬과 행 번호 추가를 동시에 진행하는 함수 (2단으로 줄일 수 있어서 간단한쿼리작성가능)
--    RANK()와 쿼리문 형태는 똑같음
--> 실무에서는축약버전사용해야하지만정석을알아야함
...
-- 3) RANK() 함수
--    정렬 후 순위 매기는 함수
--    목록 가져오기에서는 부적절함(동점자 처리 때문에 가져오는 목록의 수가 매번 달라질 수 있음)
SELECT A.EMP_NO, A.NAME, A.SALARY
...
/* 내가쓴답 잘못된부분찾아보기
SELECT B.ROW_NUM, B.EMP_NO, B.NAME, B.HD
  FROM (SELECT ROWNUM AS ROW_NUM, A.EMP_NO, A.NAME, A.HD
          FROM (SELECT EMP_NO, NAME, TO_CHAR(SYSDATE, 'YYYY-MM-DD')
                  FROM EMPLOYEE
                 ORDER BY HD ASC) A) B
 WHERE TO_CHAR(SYSDATE, 'YYYY-MM-DD') BETWEEN 3 AND 4;
*/
...
-- 2. 테이블 복사할 때 사용
-- 3. NOT NULL 제약조건 제외한 제약조건(UNIQUE, CHECK 등)은 복사되지 않음
-- 4. 형식
...
-- 제약조건 확인 / 현재 접속한 사용자가 사용가능한 제약조건은 USER_CONSTRAINTS에 있음 
DESC USER_CONSTRAINTS;
--> CONSTRAINT_NAME, TABLE_NAME 만 NOT NULL
...
-- 2. DEPARTMENT 테이블의 구조만 복사하기(모든 행 제외하고 복사하기)
-- 만족하는 행이 하나도 없는 조건을 명시
DROP TABLE DEPARTMENT2;
CREATE TABLE DEPARTMENT2
    AS (SELECT DEPT_NO, DEPT_NAME, LOCATION
          FROM DEPARTMENT
         WHERE 1 = 2);   --> 만족하는 행이 없음
...
-- 사원번호 1004번 사원의 정보를 1001번 사원의 정보로 바꾸기
UPDATE EMPLOYEE2 
