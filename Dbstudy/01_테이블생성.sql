...
/*
    ...
    NUMBER(p, s) : 정밀도(p), 스케일(s)로 표현되는 숫자 타입
        - 정밀도(p) : 정수 + 소수점 모두 포함하는 전체 유효 숫자(0은 제와)
        - 스케일(s) : 소수점 자리 수 (소수점 몇자리냐)
        예시)
...
/*
    ...
    2. USER_CONSTRAINTS 테이블 현재접속한 사용자가 만든 테이블
    3. ALL_CONSTRAINTS 테이블
*/

-- USER CONSTRAINTS 테이블의 구조 확인
DESCRIBE SYS.USER_CONSTRAINTS;    --SYS. 생략가능

-- USER_CONSTRAINTS 테이블의 CONSTRAINT_NAME 칼럼 확인
SELECT CONSTRAINT_NAME FROM USER_CONSTRAINTS;       -- 제약조건 이름 확인하는거
