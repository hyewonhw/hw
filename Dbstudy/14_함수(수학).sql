...
SELECT
       TRUNC(1.9999)      -- 1       / 정수로 절사
     , TRUNC(1.9999, 1)   -- 1.9     / 소수 한자리 남기고 절사
     , TRUNC(1.9999, 2)   -- 1. 99   / 소수 두자리 남기고 절사
  FROM 
       DUAL;
SELECT
...
--    4) 정수      : CEIL(값 * 1)     / 1          0
--    5) 일의자리  : CEIL(값 * 0.1)   / 0.1         -1      --> CEIL(111 * 0.1) / 0.1 = 120
--    6) 십의자리  : CEIL(값 * 0.01)  / 0.01        -2
--    7) 백의자리  : CEIL(값 * 0.001) / 0.001       -3
--
--    일반화       : CEIL(값 * POWER(10, DIGIT)) / POWER(10, DIGIT)
SELECT
       CEIL(1.111 * POWER(10, 1)) / POWER(10, 1)      -- 1.2    --> CEIL(1.111 * 10) / 10 = 1.2
     , CEIL(1.111 * POWER(10, 2)) / POWER(10, 2)      -- 1.12   --> CEIL(1.111 * 100) / 100 = 1.12
     , CEIL(11111 * POWER(10, -1)) / POWER(10, -1)    -- 11120  --> CEIL(
