...
public class Library {
	...
	public Library() {		//Library생성과 동시에 생성되게 만들기
	...
	private void addBook() {
		...
		books[idx++] = book;		// books[99++]일때 까지만 정상작동함
	}
	...
	private void findBook() {
		...
		for(int i = 0; i< idx; i++) {
			...
			if(books[i].getTitle().equals(title)) {		//반대로 해도됨 -> title.equals(books[i].getTitle())
				System.out.println(books[i]);
				return;		// findBook()메소드 종료 // break; 넣으면 for문이 끝나서 
			}
		}
		System.out.println("제목이 " + title + "인 책은 없습니다.");	// 이자리가 찾는책이 없다 (for문이 끝남)
	}
