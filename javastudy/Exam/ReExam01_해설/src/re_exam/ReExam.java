...
public class ReExam {
	...
	public static void q1() {
		// 조건연산자 형식 굉장히 많이씀 (if else 대신) 잘알아두셈~ 
		// (조건) ? 만족 : 불만족
	...
	public static void q3() {
		int width = 5;
		int height = 3;
		double area = (width * height) * 0.5; // 0.5인 이유는 int 값을 double 로 바꿔줘야하기 때문
	...
	public static void q4() {
		...
		int n = (int)(Math.random() * 9) + 1;  // 9개중 하나 , 시작은 1
		Scanner sc = new Scanner(System.in);
		System.out.println(dan + "x" + n + "? >>> ");
		int answer = sc.nextInt();
		System.out.println(dan * n == answer ? "정답" : "땡");  // 반복문 안에 넣어서 계속 물어보게 할 수도 있음

		/*
		Scanner sc = new Scanner(System.in);
		System.out.println((Math.random() * 10) + "x" + (Math.random() * 10) + "? >>> ");
	...
	// 1. 슬라이드 : 120 이상 탑승 가능  120 ~ 129
	// 2. 부메랑고 : 130 이상 탑승 가능  130 ~ 139
	// 3. 인피닛풀 : 140 이상 탑승 가능  140~
	// 예시(키가 125.5인 경우)
	// 슬라이드 탑승 가능합니다.
	...
	public static void q9() {
		...
		int nowYear = calendar.get(Calendar.YEAR);    // 2022
