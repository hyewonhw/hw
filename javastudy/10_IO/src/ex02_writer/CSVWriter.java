...
public class CSVWriter {
	...
	public static void main(String[] args) {
		...
		// 2. 콤마로 분리된 값들
		// 3. 확장자 .csv인 파일(기본 연결 프로그램 - 엑셀, 메모장으로 오픈가능)
		...
		List<List<String>> list = Arrays.asList(header, product1, product2, product3);		// 1차원배열이 여러개 있는것 2차원배열같은거임
		...
		try {
			...
			for(int i = 0, length = list.size(); i < length; i++) {
				List<String> line = list.get(i); 		// Array list는 list[i] 아닌거 확인하기
