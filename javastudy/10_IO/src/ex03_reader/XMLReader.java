...
public class XMLReader {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Document document = builder.parse(file); // product.xml을 파싱(분석)한 document객체
			...
			for(int i = 0; i < nodeList.getLength(); i++) {
			 	...
			 	if(node.getNodeType() == Node.ELEMENT_NODE) {		// 노드가 줄바꿈이냐 태그냐(element) 물어보는코드 (줄바꿈 #text 제외되고 <product>태그만 남음
			 		NodeList nodeList2 = node.getChildNodes();		// <product> 태그의 자식노드 (줄바꿈  <number>, <name>, <price>) 
