...
public class Main2_JSON {
	...
	public static void m2() {
		...
		StringBuilder sb = new StringBuilder();
		try(BufferedReader br = new BufferedReader(new FileReader(file))) {		// BufferedReader는 혼자 못써서 FileReader 함께 사용
			String line = null;
		 ...
		 */	
		JSONObject obj = new JSONObject(sb.toString());		// obj는 시작(={)부터 끝(=})	
		JSONObject obj2 = obj.getJSONObject("response");	// obj2는 response의 시작(={)부터 끝(=})
		JSONObject obj3 = obj2.getJSONObject("header");		// obj3은 header의 시작(={)부터 끝(=})
	...
	public static void m4() {
		...
		JSONObject obj = new JSONObject(m3());					// 겍체
		JSONObject response = obj.getJSONObject("response");	// 객체
		JSONObject body  = response.getJSONObject("body");		// 객체
		JSONObject items  = body.getJSONObject("items");		// 겍체
		JSONArray item = items.getJSONArray("item");			// 배열 -> for문
	...
	public static String m5() {
		...
		try {
			urlBuilder.append("http://apis.data.go.kr/B553077/api/open/sdsc2/storeZoneOne");		// 요청URL
			urlBuilder.append("?serviceKey=").append(URLEncoder.encode(serviceKey, "UTF-8"));
	...
	public static void m6() {
		...
		// map.put(columns.getString(0), item.get(p[0]));		// 9940
		// mao.put(columns.getStirng(1), item.get(p[1]));	// 철산역
